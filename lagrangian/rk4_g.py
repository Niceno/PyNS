...
def rk4_g( vel, vel_p, rho, mu, dt ):
    ...
    Re_p = (rho * (d) * (np.abs(vel - vel_p))) / mu
    ...
    k1 = (f * (vel - vel_p) / tau) - 9.81
    inter = vel_p + k1 * (dt / 2)
    
    
    k2 = (f * (vel - inter) / tau) - 9.81
    
    # Append to the intermediate values.
    inter = append(inter, vel_p + k2 * (dt / 2))
    
    k3 = (f * (vel - inter[1]) / tau) - 9.81
    
    # Append to the intermediate values.
    inter = append(inter, vel_p + k3 * dt)
    
    k4 = (f * (vel - inter[2]) / tau) - 9.81
