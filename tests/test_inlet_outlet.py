...
def main(show_plot=True):
    ...
    planes = ('XY', 'XZ', 'YZ')
    tests = array([11, 12, 13, 14, 21, 22, 23, 24,
    ...
    if TEST == 11:
        ...
        for k in range(0, nz):
            uf.bnd[W].val[0, 3 * ny // 4:ny, k] = + \
                par(0.01, yn[3 * ny // 4:ny + 1])
            uf.bnd[W].typ[0, :ny // 4, k] = OUTLET

    elif TEST == 32:  # vertical mirror of 31
        for k in range(0, nz):
            uf.bnd[E].val[0, 3 * ny // 4:ny, k] = - \
                par(0.01, yn[3 * ny // 4:ny + 1])
            uf.bnd[E].typ[0, 0:ny // 4, k] = OUTLET

    elif TEST == 33:  # rotated 31
        for k in range(0, nz):
            vf.bnd[S].val[3 * nx // 4:nx, 0, k] = + \
                par(0.01, xn[3 * nx // 4:nx + 1])
            vf.bnd[S].typ[:nx // 4, 0, k] = OUTLET

    elif TEST == 34:  # horizontal mirror of 33
        for k in range(0, nz):
            vf.bnd[N].val[3 * nx // 4:nx, 0, k] = - \
                par(0.01, xn[3 * nx // 4:nx + 1])
            vf.bnd[N].typ[:nx // 4, 0, k] = OUTLET
